...
fun main() {
    // part one
    val part1 = day1.lines()
        .sumOf { line -> line.last { it.isDigit() }.digitToInt() * 10 + line.first { it.isDigit() }.digitToInt() }
    println(part1)

    // part two
    val part2 = day1.lines().sumOf {
        words.foldIndexed(it) { idx, acc, ele -> acc.replace(ele, "$ele${idx + 1}$ele") }.filter(Char::isDigit)
            .map(Char::digitToInt).run { first() * 10 + last() }
    }
    println(part2)
}