const val day2 = """Game 1: 4 blue; 1 green, 2 red; 4 blue, 1 green, 6 red
...
data class Game(
    val id: Int, val shows: List<Show>
)
...
fun main() {
    // part 1
    val games = day2.lines().mapIndexed { index, line ->
        Game(index + 1, line.split("""[:;]""".toRegex()).drop(1).map { show ->
            val (blue, green, red) = "blue green red".split("""\s+""".toRegex()).map { color ->
                """(\d+)\s+$color""".toRegex().find(show)?.let {
    ...
    val part1 =
        games.filter { it.shows.all { bag -> bag.red <= 12 && bag.green <= 13 && bag.blue <= 14 } }.sumOf { it.id }
    println(part1)


    // part 2
    val part2 =
        games.sumOf { game -> (game.shows.maxOf { it.blue } * game.shows.maxOf { it.red } * game.shows.maxOf { it.green }) }
    println(part2)
